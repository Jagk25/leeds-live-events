...
function walkJson(node, source, out, depth = 0) {
  ...
  if (title && url && (types.includes('Event') || start || source === 'meetup')) {
    ...
    pushEvent(out, {
      source, title, url, startAt: start, endAt: node.endDate || node.end_date,
      venue: loc.name || loc.venue, address: loc.address?.addressLocality || loc.city || loc.address || 'Leeds',
      description: node.description, image: node.image?.url || node.image,
    });
  }
